...
template <typename T>
inline void b2DynamicTree::Query(T *callback, const b2AABB &aabb,
                                 int groupId) const {
 ...
 *******/
template <typename T>
inline void b2DynamicTree::RayCast(
    T *callback, const b2RayCastInputAABB &input,
    int groupId) const // Group ID is not the part of the original code
{
  b2Vec2 p1 = input.p1;
  b2Vec2 p2 = input.p2;
  b2Vec2 r = p2 - p1;
